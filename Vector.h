...
template<typename T>
size_t Vector<T>::get_capacity() const { return capacity_; }

template<typename T>
size_t Vector<T>::get_size() const { return size_; }

template<typename T>
bool Vector<T>::is_empty() const { return size_ == 0; }

template<typename T>
void Vector<T>::reserve(const size_t value)
{
    capacity_ = value;
...
template<typename T>
void Vector<T>::assign(const size_t& quantity, const T& value)
{
    T* arr;
    arr = new T[quantity];
    size_ = quantity;
...
template<typename T>
size_t Vector<T>::count(const T& value)
{
    size_t counter = 0;
...
template<typename T>
T& Vector<T>::operator=(const std::vector<T>& v)
{
    this->size_ = v.size();
    T* arr = new T[v.size()];
...
template<typename T>
bool Vector<T>::operator==(const Vector& another)
{
